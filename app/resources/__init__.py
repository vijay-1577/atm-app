...
from app.models import AtmCard
...
def create_or_update_resource(**kwargs):
    ...
    try:
        ...
        if kwargs["create"]:
            if kwargs["resource_type"] == "AtmCard":
                # Get AtmCard's auth token
                AtmCard = AtmCard.query.filter_by(
                    AtmCardname=kwargs["AtmCardname"]).first()
                auth_token = AtmCard.generate_auth_token(AtmCard.id)
...
def before_request():
    ...
    AtmCard registration, login and index.
    """
    ...
    if request.endpoint in ["studentlistapi", "studentapi", "teacherlistapi",
        ...
        if token is not None:
            token_decode_response = AtmCard.decode_auth_token(token)
            try:
                AtmCard = AtmCard.query.get(token_decode_response)
                g.AtmCard = AtmCard
            except:
